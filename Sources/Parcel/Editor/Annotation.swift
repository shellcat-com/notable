...
enum Tool: String, CaseIterable, Identifiable {
    case select, arrow, rectangle, ellipse, text, pencil, censor
...
enum CensorMode: String, CaseIterable, Identifiable, Equatable, Codable {
    ...
    case erase      // local fill sampled from surrounding Capture pixels
...
enum Handle: Equatable {
    ...
    case rotate                                     // rotation affordance above selection
}
...
struct Annotation: Identifiable, Equatable, Codable {
    ...
    /// Radians, applied around the kind's bounding-box center when rendering.
    var rotation: CGFloat

    init(id: UUID = UUID(), kind: AnnotationKind, style: AnnotationStyle, rotation: CGFloat = 0) {
        self.id = id
        self.kind = kind
        self.style = style
...
extension AnnotationKind {
    ...
    private enum Kind: String, Codable {
        case arrow, rectangle, ellipse, censor, text, pencil, number, stamp, highlight, measure, spotlight
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        case .ellipse:
            self = .ellipse(rect: try container.decode(CGRect.self, forKey: .rect))
        case .censor:
...
extension AnnotationKind {
    ...
    var boundingBox: CGRect {
        ...
        case let .rectangle(rect), let .ellipse(rect), let .censor(rect), let .text(rect, _),
    ...
    func hitTest(
        ...
        case .rectangle, .ellipse, .censor, .text, .stamp, .spotlight:
    ...
    var handles: [(Handle, CGPoint)] {
        ...
        case .rectangle, .ellipse, .censor, .text, .stamp, .spotlight, .number:
            let r = boundingBox
            let rotatePoint = CGPoint(x: r.midX, y: r.minY - 24)
            return [
    ...
    func translated(dx: CGFloat, dy: CGFloat) -> AnnotationKind {
        ...
        case let .ellipse(rect):
            return .ellipse(rect: rect.offsetBy(dx: dx, dy: dy))
        case let .censor(rect):
    ...
    func resized(handle: Handle, to point: CGPoint) -> AnnotationKind {
        ...
        case let .ellipse(rect):
            return .ellipse(rect: Self.resize(rect, handle: handle, to: point))
        case let .censor(rect):
